...
from utils import ReplayBuffer, PrioritizedReplayBuffer, NoisyLinear

from model_rainbow_dqn import Network_dqn
from model_rainbow_lstm import Network_lstm
from model_rainbow_transformer import Network_tf
...
class Agent:
    ...
    def __init__(
        ...
        if self.select_model == 'dqn':
            self.dqn = Network_dqn(
                obs_dim, action_dim, self.atom_size, self.support
            ).to(self.device)
            self.dqn_target = Network_dqn(
                obs_dim, action_dim, self.atom_size, self.support
            ).to(self.device)
            self.dqn_target.load_state_dict(self.dqn.state_dict())
            self.dqn_target.eval()
        if self.select_model == 'lstm':
            self.dqn = Network_lstm(
                obs_dim, action_dim, self.atom_size, self.support
            ).to(self.device)
            self.dqn_target = Network_lstm(
                obs_dim, action_dim, self.atom_size, self.support
            ).to(self.device)
            self.dqn_target.load_state_dict(self.dqn.state_dict())
            self.dqn_target.eval()
        if self.select_model == 'tf':
            self.dqn = Network_tf(
                obs_dim, action_dim, self.atom_size, self.support
            ).to(self.device)
            self.dqn_target = Network_tf(
                obs_dim, action_dim, self.atom_size, self.support
            ).to(self.device)
            self.dqn_target.load_state_dict(self.dqn.state_dict())
            self.dqn_target.eval()
    ...
    def train(self, num_frames: int, plotting_interval: int = 200):
        ...
        for frame_idx in range(1, num_frames + 1):
            ...
            writer.add_scalar("Dist/frame_idx", info["x_pos"], frame_idx)
